  ...
  function getLocations()
  {
    ...
    db.collection("locations").get().then(function(querySnapshot) {
      querySnapshot.forEach(function(doc) {
        locs.push({value: [doc.get("name"),doc.get("address")]});
      });
    });
    return locs;
  }
  ...
  return (
    ...
    <ScrollView style={styles.container}>
      ...
      <View>
        ...
        <View style={{flexDirection:"row", marginBottom:hp("2%"), marginLeft:wp("5%")}}>
        <Dropdown
        label='Pickup Location'
        containerStyle={{width:wp("70%")}}
        itemCount={3}
        data={getLocations()}
        onChangeText={(value, index, data) => { pickup =value[1]}}
      ...
      />
           ...
           <TouchableOpacity style={{backgroundColor:"rgba(106,164,27,1)", width:wp("20%"), alignItems:"center", marginLeft:wp("5%") , padding:10, borderRadius:10, position:"absolute",right:wp("1%")}}
                onPress={() => {navigation.navigate("Shopping Cart",{pickUp:pickup})}}
          >
              <Text style={{color:"white"}}>
