...
const MISSING_CONTEXT_ERROR = "We couldn't find a navigation context. Have you wrapped your app with 'NavigationContainer'? See https://reactnavigation.org/docs/en/getting-started.html for setup instructions.";
...
const BaseNavigationContainer = React.forwardRef(function BaseNavigationContainer(_ref, ref) {
  ...
  const [state, setNavigationState] = React.useState(() => getPartialState(initialState == null ? undefined : initialState));
  ...
  const performTransaction = React.useCallback(callback => {
  ...
  const getState = React.useCallback(() => transactionStateRef.current !== null ? transactionStateRef.current : navigationStateRef.current, []);
  const setState = React.useCallback(navigationState => {
